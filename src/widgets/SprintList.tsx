...
export default function SprintList({agile, search, selectedCustomFields, hideFinishedSprints, sprints, setSprints, setIssues}:
    ...
    const loadIssuesOfSprint = useCallback(async (sprint: Sprint) => {
        return await host.fetchYouTrack(`agiles/${agile.id}/sprints/${sprint.id}/issues?fields=id,idReadable,resolved,summary,customFields(name,value(name)),project(id,name),isDraft&$top=-1`)
            .then((issues: Issue[]) => {
